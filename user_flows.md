...
8. Keyboard focus moves to the Add-medication FAB immediately after a delete succeeds, since the deleted row's own controls (including the Delete control that had focus) no longer exist in the DOM — mirroring the app's general discipline of never leaving focus on a control that no longer exists.
